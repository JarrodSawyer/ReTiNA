...
if len(sys.argv) != 2:
	print("usage: python trafficStats.py teams.cfg")
	exit();
...
def createTeamStatsList(team_info):
	...
	for item in team_info:
		team_stats.append( [item[0], 0, 0, 0, 0] );	#create a list element containing team_name, 0 (incoming), 0 (outgoing), 0 (outgoingtotal), and 0 (incomingtotal)
	return team_stats
...
def resetTeamStats():
	...
	for item in team_stats_list:
		item[1] = 0
		item[2] = 0             #reset incoming & outgoing counters to zero. Do not reset Total Outgoing and Total Incoming counts
...
def incrementPacketCount(subnet, type):
	#print ("CALLING INCREMENT PACKET COUNT")
	...
	if team != "":
		for stats_item in team_stats_list:	#find the stats for the team and add to packet count
			...
			if stats_item[0] == team:
				...
				if type == 1:	#if we're adding to Incoming
					stats_item[4] += int(numPackets)
				#print(stats_item)
...
def pushTrafficStats(dbwrapper):
	...
	for team_item in team_info_list:
		...
		for stats_item in team_stats_list:
			if stats_item[0] == team_name:
				...
				total_incoming = stats_item[4]
				now = time.time()
				#print("sending stats for team %s" % team_name)
				dbwrapper.addStats(now, team_name, incoming, outgoing, total_outgoing, total_incoming)
...
def parseTrafficStats():
	...
	while 1:
		...
		#packetInfo = re.search( 'length (\d*)\n', line)	#parse the number of packets send
		...
		packetInfo = re.search( 'length (\d*)\n', line)
	        ...
	        #print(packetInfo)
		if packetInfo:
			#print (str(packetInfo.group()))
			packetInfo_string = str(packetInfo.group())
			print (packetInfo_string)
			space_index = packetInfo_string.find(" ")
			numPackets = packetInfo_string[ space_index+1:len(packetInfo_string)-1 ]
			#print (numPackets)
			if int(numPackets) > 0:	#only continue if there were actually send packets
				...
				if int(now - start) >= loopTime:	#if the set time interval has passed, push the current traffic stat data and reset packet counters
					pushTrafficStats(dbwrapper)
					resetTeamStats()
					start = time.time()
		else:
			print ("No Packet Info")
...
parseTrafficStats()						#this function call gets the process going
