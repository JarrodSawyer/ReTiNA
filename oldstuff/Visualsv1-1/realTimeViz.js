...
refreshData = function()
{
	...
	/**blue_white = Math.floor(1 + Math.random()*1000);
	blue_red = Math.floor(1 + Math.random()*1000);
	
	red_white = Math.floor(1 + Math.random()*1000);
	red_blue = Math.floor(1 + Math.random()*1000);
	
	white = Math.floor(1 + Math.random()*1000); **/
	
	$.ajax({
		type: "GET",
		url: "packet.xml", //http://10.0.0.1/cgi-bin/StatsReport.cgi
		dataType: "xml",
...
{	
	...
	//circles
	blueToWhiteCir = lgBlue.circle(time_display[time_display.length - 1], bytes_blueToWhite_display[bytes_blueToWhite_display.length - 1], 10);
	blueToWhiteCir.attr({stroke: "none", fill: "white"});
	blueToRedCir = lgBlue.circle(time_display[time_display.length - 1], bytes_blueToRed_display[bytes_blueToRed_display.length - 1], 10);
	blueToRedCir.attr({stoke: "none", fill: "#640A0A"});
	
	redToWhiteCir = lgRed.circle(time_display[time_display.length - 1], bytes_redToWhite_display[bytes_redToWhite_display.length - 1], 10);
	redToWhiteCir.attr({stroke: "none", fill: "white"});
	redToBlueCir = lgRed.circle(time_display[time_display.length - 1], bytes_redToBlue_display[bytes_redToBlue_display.length - 1], 10);
	redToBlueCir.attr({stroke: "none", fill: "#10425D"});
	
	whiteCir = lgWhite.circle(time_display[time_display.length - 1], bytes_White_display[bytes_White_display.length - 1], 10);
